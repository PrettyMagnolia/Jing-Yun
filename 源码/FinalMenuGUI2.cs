...
public class FinalMenuGUI2 : MonoBehaviour
{
    public static bool DisplayEndMenu2 = false;//��Ϸ�Ƿ��Ѿ�����
    ...
    {
        FinalSet();
        FinalScore2 = ScoreTextScript2.score2;
    }
    ...
    void OnGUI()
    {
        ...
        if (DisplayEndMenu2)
        {
            int Box_Width = Screen.width/2, Box_Height = Screen.height/2;

            GUI.Box(new Rect(Screen.width / 2 - Box_Width / 2, Screen.height / 2 - Box_Height / 2, Box_Width, Box_Height), "游戏结束");
            GUI.Label(new Rect(Screen.width / 2 - 55, Screen.height / 2 - 75, 300, 25), "最终得分：" + FinalScore2.ToString());
            if (menuPage == "final")
            {
